...
async def test_remuneration_repo_search_filters(db_session: AsyncSession):
    ...
    r1 = RemunerationCollected(
        execution_id=exec_an.id, monthly_execution_id=monthly.id,
        nome_servidor="TEST_USER", cpf_servidor="123", 
        ano_exercicio=2025, mes_referencia="01",
        valor_bruto=100, codigo_identificacao="ID_COV", 
        codigo_matricula="M_COV", raw_payload_json="{}"
    )
    db_session.add(r1)
